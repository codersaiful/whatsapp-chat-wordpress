...
function cat_wa_button_render($attributes)
{
	if ($attributes['isSelectedAccount'] != -1) {
        return do_shortcode("[catwa_button id={$attributes['isSelectedAccount']}]");
    } else {
        ...
        if (empty($attributes['imageUrl'])) {
            $btn_icon_or_image = '<div class="wa__btn_icon"><img src="' . CAT_WHATSAPP_PLUGIN_URL . 'assets/img/whatsapp_logo.svg' . '" alt=' . $attributes['imageAlt'] . '/></div>';
        } else {
...
function cat_wa_assets()
{
	wp_enqueue_style('cat-css-popup', CAT_WHATSAPP_PLUGIN_URL . 'assets/dist/css/style.css');
}

// Hook: Block assets.
add_action('init', 'cat_wa_block_assets');
add_action('enqueue_block_assets', 'cat_wa_assets');
