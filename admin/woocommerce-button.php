...
?>
<p><?php echo __('Display WhatsApp click to chat button on WooCommerce single product pages.', 'codeastrology-whatsapp') ?></p>
<table class="form-table">
    <tbody>
        <tr>
            <th scope="row"><label for="cat-wa-switch-control"><?php echo __('Enabled', 'codeastrology-whatsapp') ?></label></th>
            <td>
                <div class="cat-wa-switch-control">
                    <input type="checkbox" id="cat-wa-switch" name="isShow" <?php checked($option['isShow'], 'ON') ?>>
                    <label for="cat-wa-switch" class="green"></label>
                </div>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="position"><?php echo __('Button position', 'codeastrology-whatsapp') ?></label></th>
            <td>
                <div class="cat-wa-d-row cat-wa-border-box">
                    <div class="cat-wa-radio-select-img">
        ...
        <tr>
            <th scope="row"><label for="selected_accounts"><?php echo __('Select accounts to display', 'codeastrology-whatsapp') ?></label></th>
            <td>
                <div id="app-woo"></div>
            </td>
        </tr>
...
<script type="text/template" id="accountListTemplate">
    <label class="cat-list-status">
        <strong>
            <% if (_.isEmpty(activeAccounts)) { %>
            <?php echo __('Please select accounts you want them to display in WhatsApp Chat Widget', 'codeastrology-whatsapp') ?>
            <% } else { %>
            <?php echo __('Selected Accounts:', 'codeastrology-whatsapp') ?>
            <% } %>
        </strong>
    </label>
    <% if (!_.isEmpty(activeAccounts)) { %>
    <div class="cat-list-box-accounts postbox" id="sortable">
        <% _.each(activeAccounts, function (account) { %>
            <div class="cat-list-items" data-index="<%= account.accountId %>" data-position="<%= account.wc_position %>">
                <div class="box-content box-content-woo">
                    <div class="box-row">
                        ...
                        <div class="icon-block">
                            <img src="<?php echo CAT_WHATSAPP_PLUGIN_URL . 'assets/img/bar-sortable.svg' ?>" width="20px">
                        </div>
                    </div>
                </div>
            </div>
