        ...
        public function add_notification() {
            if (function_exists('get_current_screen')) {
                if (get_current_screen()->id == 'plugins' || get_post_type() == 'cat_wa_accounts' ) {
                    ...
                    <script>
                    jQuery("#cat_wa-review a").on("click", function () {
                        var thisElement = this;
                        var fieldValue = jQuery(thisElement).attr("data");
                        var link = "https://codecanyon.net/item/whatsapp-chat-wordpress/reviews/22800580";
                        var hidePopup = false;
                        ...
                        jQuery.ajax({
                            ...
                            type: "post",
                            data: {
                                action: 'cat_wa_save_review',
                                field: fieldValue,
                                nonce: '<?php echo esc_attr(wp_create_nonce("cat_filebird_cross_nonce")) ?>',
                            },
                            }).done(function (result) {
                            if (result.success) {
                                if (hidePopup == true) {
                                    jQuery('#cat_wa-review').hide("slow");
                                }
                            } else {
