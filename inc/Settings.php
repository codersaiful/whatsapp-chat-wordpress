...
{
    protected $option;
    protected $option_group = 'cat_whatsapp_group';
    protected $option_design = 'cat_whatsapp_design';
    protected $option_button_group = 'cat_whatsapp_button_group';
    protected $option_woo_button_group = 'cat_wa_woo_button_group';
    protected $option_ga_group = 'cat_wa_ga_group';
    ...
    private function runHooks(){
        ...
        add_action('wp_ajax_cat_wa_set_account_position', [$this, 'set_account_position']);
        add_action('wp_ajax_cat_wa_load_accounts_ajax', [$this, 'load_accounts_ajax']);
        add_action('wp_ajax_cat_wa_set_account_status', [$this, 'set_account_status']);

        add_action('wp_ajax_cat_wa_save_display_setting', [$this, 'save_display_setting']);
        add_action('wp_ajax_cat_wa_save_design_setting', [$this, 'save_design_setting']);
        add_action('wp_ajax_cat_wa_save_woocommerce_setting', [$this, 'save_woocommerce_setting']);
        add_action('wp_ajax_cat_wa_save_analytics_setting', [$this, 'save_analytics_setting']);
        add_action('wp_ajax_cat_wa_save_url_setting', [$this, 'save_url_setting']);
        add_action('wp_ajax_cat_wa_get_posts', [$this, 'get_posts']);

        add_filter('plugin_action_links_' . CAT_WHATSAPP_BASE_NAME, [$this, 'addActionLinks']);
    ...
    public function addActionLinks($links)
    {
        $links = array_merge(array(
            '<a href="' . esc_url(admin_url('/admin.php?page=cat_whatsapp_floating_widget')) . '">' . __('Settings', 'codeastrology-whatsapp') . '</a>',
        ), $links);
    ...
    public function pluginRowMeta($links, $file){
        if ( strpos( $file, 'whatsapp.php' ) !== false ) {
            $new_links = array(
              'doc' => '<a href="https://codeastrology.com/wordpress-whatsapp-chat-tutorial/" target="_blank">'. __("Documentation", "filebird") .'</a>'
            );
    ...
    public function admin_menu()
    {
        ...
        $this->floatingWidgetSlug = add_submenu_page('cat_whatsapp', __('Floating Widget', 'codeastrology-whatsapp'), __('Floating Widget', 'codeastrology-whatsapp'), 'manage_options', 'cat_whatsapp_floating_widget', [$this, 'floating_widget_view']);
        $this->settingSlug = add_submenu_page('cat_whatsapp', __('Settings', 'codeastrology-whatsapp'), __('Settings', 'codeastrology-whatsapp'), 'manage_options', 'cat_whatsapp_setting', [$this, 'create_page_setting_widget']);
    }
    ...
    public function admin_enqueue_scripts($hook_suffix)
    {
        ...
        wp_register_style('cat-css', CAT_WHATSAPP_PLUGIN_URL . 'assets/css/admin.css', ['wp-color-picker']);
        wp_enqueue_style('cat-css');

        wp_register_style('cat-tippy-css', CAT_WHATSAPP_PLUGIN_URL . 'assets/css/tooltip.css');
        wp_enqueue_style('cat-tippy-css');

        wp_dequeue_style('woosea_jquery_ui-css');
        
        wp_register_style('cat-wa-widget', CAT_WHATSAPP_PLUGIN_URL . 'assets/dist/css/style.css');
        wp_enqueue_style('cat-wa-widget');
        wp_enqueue_style('ui-range', CAT_WHATSAPP_PLUGIN_URL . 'assets/libs/ui-range.css');
        ...
        wp_register_script(
            'cat-wa-js', 
            CAT_WHATSAPP_PLUGIN_URL . 'assets/dist/js/app.js',
            [
                'jquery', 
                'wp-color-picker', 
                'backbone', 
        ...
        wp_localize_script('cat-wa-js', 'cat_wa', [
            ...
            'timezone' => $timezone,
            'i18n' => array(
                'select_post' => __('Select posts to display the widget', 'codeastrology-whatsapp')
            )
        ]);
        wp_enqueue_script('cat-wa-js');
        wp_enqueue_script('jquery-validate', CAT_WHATSAPP_PLUGIN_URL . 'assets/libs/jquery.validate.min.js');
        wp_enqueue_script('select2-js', CAT_WHATSAPP_PLUGIN_URL . 'assets/libs/select2.full.min.js');
        wp_enqueue_style('select2-css', CAT_WHATSAPP_PLUGIN_URL . 'assets/libs/select2.min.css');
    }
    ...
    {
        register_setting($this->option_group, 'cat_whatsapp_setting');
        register_setting($this->option_design, 'cat_whatsapp_setting');
        register_setting($this->option_woo_button_group, 'cat_wa_woobutton_setting', [$this, 'save_woobutton_setting']);
        register_setting($this->option_ga_group, 'cat_wa_ga_setting', [$this, 'save_ga_setting']);
        ...
        add_settings_section('cat_woocommerce_button', '', [$this, 'woocommerce_button_callback'], 'settings-whatsapp-1');
        add_settings_section('cat_analytics', '', [$this, 'analytics_callback'], 'settings-whatsapp-2');
        add_settings_section('cat_url', '', [$this, 'url_callback'], 'settings-whatsapp-3');
    }
    ...
    public function save_woobutton_setting()
    {
        ...
        $new_input['cat_woo_button_position'] = sanitize_text_field($_POST['cat_woo_button_position']);
        $new_input['cat_woo_button_status'] = isset($_POST['cat_woo_button_status']) ? 'ON' : 'OFF';
        return $new_input;
    }

    public function save_ga_setting()
    {
        if (isset($_POST['cat_wa_ga_status'])) {
            return '1';
        }
        return '0';
    }
    ...
    public function save_display_setting()
    {
        ...
        update_option('cat_wa_widget_display', $new_input);
    ...
    public function save_design_setting()
    {
        ...
        update_option('cat_wa_widget_styles', $new_input);
        wp_send_json_success();
    }
    ...
    public function save_woocommerce_setting(){
        ...
        update_option('cat_wa_woocommerce', $new_input);
        wp_send_json_success();
    }
    ...
    public function save_analytics_setting(){
        ...
        update_option('cat_wa_analytics', $new_input);
        wp_send_json_success();
    }
    ...
    public function save_url_setting(){
        ...
        update_option('cat_wa_url', $new_input);
        wp_send_json_success();
    }
    ...
    public function set_account_position()
    {
        ...
        foreach ($positions as $index => $id) {
            update_post_meta($id, "cat_wa_{$type}", $index);
        }
    ...
    public function load_accounts_ajax()
    {
        ...
        $results = array_map(function ($account) {
            ...
            $wg_show = get_post_meta($account->ID, 'cat_wa_widget_show', true);
            $wg_position = get_post_meta($account->ID, 'cat_wa_widget_position', true);
            $wc_show = get_post_meta($account->ID, 'cat_wa_wc_show', true);
            $wc_position = get_post_meta($account->ID, 'cat_wa_wc_position', true);
    ...
    public function set_account_status()
    {
        ...
        $wg_position = get_post_meta($id, 'cat_wa_widget_position', true);
        $wc_position = get_post_meta($id, 'cat_wa_wc_position', true);

        if ('' === $wg_position) {
            update_post_meta($id, "cat_wa_widget_position", 0);
        }

        if ('' === $wc_position) {
            update_post_meta($id, "cat_wa_wc_position", 0);
        }

        update_post_meta($id, "cat_wa_{$type}", $status);
        wp_send_json_success();
    }
    ...
    public function get_posts(){
		...
		foreach ( $search_results_query->get_posts() as $post ) {
			$posts_results[ $post->ID ] = sprintf(
				__( '%1$s (ID: %2$s)', 'codeastrology-whatsapp' ),
				get_the_title( $post ),
				$post->ID
			);
		}
