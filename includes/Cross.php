...
if ( !class_exists('NjtCross') ) {
    class NjtCross
    {
        ...
        public function doHooks() {
            add_action('init', function(){
                if (!$this->is_plugin_exist()) {
                    ...
                    $popupOption        = get_option("cat_popup_{$this->pluginPrefix}_cross"); //Save the next time notification will appear
         
                    if ($notificationOption === false || time() >= $notificationOption) {
                        add_action('admin_notices', array($this, 'add_notification'));
                        add_action("wp_ajax_cat_{$this->pluginPrefix}_cross_notification", array($this, 'ajax_set_notification'));
                    }
                    ...
                    add_action("wp_ajax_cat_{$this->pluginPrefix}_cross_install", array($this, 'ajax_install_plugin'));
                    add_action("wp_ajax_cat_{$this->pluginPrefix}_cross_hide", array($this, 'ajax_hide_cross'));
                }
            });
        }
        ...
        public function need_update_option(){ 
            ...
            update_option("cat_popup_{$this->pluginPrefix}_cross", $time);
            update_option("cat_notification_{$this->pluginPrefix}_cross", $time);
        }
        ...
        public function ajax_install_plugin(){
            check_ajax_referer("cat_filebird_cross_nonce", 'nonce', true);
        ...
        public function ajax_set_notification(){
            check_ajax_referer("cat_{$this->pluginPrefix}_cross_nonce", 'nonce', true);
            //Save after 30 days
            update_option("cat_notification_{$this->pluginPrefix}_cross", time() + (30 * 60 * 60 * 24));
            wp_send_json_success();
        }
        ...
        public function ajax_hide_cross(){
            ...
            update_option("cat_{$type}_{$this->pluginPrefix}_cross", $time); 
            wp_send_json_success();
        }
    }
}
...
if ( !class_exists('FileBirdCross') ) {
    ...
    FileBirdCross::get_instance('filebird', 'filebird+codeastrology', CAT_WHATSAPP_PLUGIN_URL, array('filebird/filebird.php', 'filebird-pro/filebird.php'));
}
