...
class Woocommerce
{
    ...
    public function init() {
        ...
        add_filter('cat_whatsapp_is_page_or_shop_filter', array($this, 'isPageOrShop'), 10, 1);
        add_filter('cat_whatsapp_get_post_id_filter', array($this, 'getPostId'), 10, 1);
    ...
    public function woocommerce_get_stock_html( $html, $product){
        $shouldDisplay = apply_filters('cat_wa_out_of_stock_display', false);
        ...
        foreach ($this->activeAccounts as $row) {
            $html .= '<div class="cat-woo-products-button">' . do_shortcode('[catwa_button id="' . esc_attr($row->ID) . '"]') . '</div>';
        }
    ...
    public function showAfterShortDescription($post_excerpt)
    {
        ...
        $btnContent = '';
        foreach ($this->activeAccounts as $row) {
            $btnContent .= '<div class="cat-woo-products-button">' . do_shortcode('[catwa_button id="' . $row->ID . '"]') . '</div>';
        }
    ...
    public function insert_button()
    {
        ...
        foreach ($this->activeAccounts as $row) {
            echo '<div class="cat-woo-products-button">' . do_shortcode('[catwa_button id="' . esc_attr($row->ID) . '"]') . '</div>';
        }
    }
}
